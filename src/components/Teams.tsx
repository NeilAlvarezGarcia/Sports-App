...
const Teams = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) =>  {
    ...
    const res = await axios.get(`https://www.thesportsdb.com/api/v1/json/2/search_all_teams.php?s=${sport}&c=${country}`);
        ...
        <ContainerAuthentication mode={mode} className='removePadding'>
            <form onSubmit={handleSubmit}>
                    <label htmlFor="country">
                        Country
                        <input type="text" id='country' autoComplete='off' placeholder='Spain' value={country} onChange={handleChange}/>
                    </label>
                    <button>
