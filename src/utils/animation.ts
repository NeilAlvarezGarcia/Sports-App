...
if (typeof window !== 'undefined') {
  requestAnimationFrame =
    ...
    (window as unknown as Record<string, unknown>).webkitRequestAnimationFrame ||
...
function rotateCircle(this: IAnimation, circles: SVGCircleElement[]) {
  ...
  for (let i = 0; i < len; ++i) {
    if (this.iteration >= CIRCLES_INTERVAL * i) {
      ...
      circles[i].setAttributeNS('', 'cx', 20 + BOUNDING * Math.sin(position) + '');
      circles[i].setAttributeNS('', 'cy', 20 + BOUNDING * Math.cos(position) + '');
      let size: number;
